...
public class Tournament {
	...
	static int model_S(int N) {
		return (2*N + 2) * (int)(Math.log(N) / Math.log(2)) - 4*N + 5;
	}
	...
	static void trial(int type) {
		...
		for (int size = 64; size <= 65536; size *= 2) {
			...
			int numLess = heap.lessCount;     // DO SOMETHING ELSE!!!
			int numExch = heap.exchCount;
	...
	static void populate (Tournament heap, int size, int type) {
		...
		if (type == BEST) {
			// Best case is if the numbers are inserted in ascending order so "less" is called n times 
			// and "exch" is called 0 times
			for (int i = 0; i < size; i++) {
				heap.insert(size - i - 1);
			}
			...
			// Worst case is if the numbers are inserted in descending order
			for (int i = 0; i < size; i++) {
				heap.insert(i);
			}
		}
